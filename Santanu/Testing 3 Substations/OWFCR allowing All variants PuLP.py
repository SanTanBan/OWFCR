...
def Euclidean_Distance_Matrix_Generator(locations_dict, Obstacles_Index_Set, Obstacles_Line_Start_Lat_Dict, Obstacles_Line_Start_Lon_Dict, Obstacles_Line_End_Lat_Dict, Obstacles_Line_End_Lon_Dict, output_filename):
    ...
    print(Obstacles_Index_Set)
    print(Obstacles_Line_Start_Lat_Dict)
    print(Obstacles_Line_Start_Lon_Dict)
    print(Obstacles_Line_End_Lat_Dict)
    print(Obstacles_Line_End_Lon_Dict)
...
if No_Cross_Constraints == 0:
    Crossings_Set = Intersecting_Arcs_Combo(All_Nodes_DictKey_Lat_Lon_Power_ValueTuple, "Crossing Segments Old Old Eq 10.xlsx")
...
f=LpVariable.dicts('f',((i,j) for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j), lowBound=0)

x=LpVariable.dicts('x',((i,j,t) for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j for t in Cables_Index_Set), cat='Binary')

y=LpVariable.dicts('y',((i,j) for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j), cat='Binary')

if OWFCR_Type==2:
    z=LpVariable.dicts('z',((j,d) for j in Turbines_Index_Set for d in D), cat='Binary')

if OWFCR_Type==3:
    q=LpVariable.dicts('q',((i,j) for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j), cat='Binary')



# Minimization Objective Function
if OWFCR_Type==0 or OWFCR_Type==1: # Equation 1 for OWFCR or SS
    prob+=lpSum(Cable_Cost_Dict[t] * Distance_Dict[(i,j)] * x[i,j,t] for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j for t in Cables_Index_Set)
    ...
    prob+=lpSum(Cable_Cost_Dict[t] * Distance_Dict[(i,j)] * x[i,j,t] for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j for t in Cables_Index_Set) + lpSum(pi_OWFCR_BR_Cost[d] * z[j,d] for d in D for j in Turbines_Index_Set)
    ...
    prob+=lpSum(Cable_Cost_Dict[t] * Distance_Dict[(i,j)] * x[i,j,t] for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j for t in Cables_Index_Set) + lpSum(min_cable_cost * Distance_Dict[(i,j)] * q[i,j] for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j)
    ...
    prob+=lpSum(Cable_Cost_Dict[t] * Distance_Dict[(i,j)] * x[i,j,t] for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j for t in Cables_Index_Set) + lpSum(c_otm * x[i,j,Tao] for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j)
...
for h in (Turbines_Index_Set | Steiners_Index_Set):
    prob += lpSum(f[h,i] - f[i,h] for i in All_Nodes_Index_Set if i!=h) == All_Nodes_DictKey_Lat_Lon_Power_ValueTuple[h][2]
...
for i in All_Nodes_Index_Set:
    for j in All_Nodes_Index_Set:
        if i!=j:
            prob += lpSum(Cable_Capacity_Dict[t] * x[i,j,t] for t in Cables_Index_Set) >= f[i,j]
#mdl.addConstrs(quicksum() >= f[i,j] for i in All_Nodes_Index_Set for j in All_Nodes_Index_Set if i!=j)
#mdl.update()

# Equation 5
for h in Turbines_Index_Set:
    prob += lpSum(y[h,j] for j in All_Nodes_Index_Set if j!=h) == 1
#mdl.addConstrs(quicksum() == 1 for h in Turbines_Index_Set)
#mdl.update()

# Equation 6
for h in Substations_Index_Set:
    prob += lpSum(y[h,j] for j in All_Nodes_Index_Set if j!=h) == 0
...
# Equation 8
for h in Steiners_Index_Set:
    prob += lpSum(y[i,h] for i in All_Nodes_Index_Set if i!=h) <= 1
#mdl.addConstrs(quicksum(y[i,h] for i in All_Nodes_Index_Set if i!=h) <= 1 for h in Steiners_Index_Set)
#mdl.update()

# Equation 9
for h in Substations_Index_Set:
    prob += lpSum(y[i,h] for i in All_Nodes_Index_Set if i!=h) <= C
        ...
        for b in All_Nodes_Index_Set:
            if a!=b:
                for f in All_Nodes_Index_Set:
                    if f!=a and f!=b:
                        prob += lpSum(y[i,j] for i,j in Clique_Arc_Subset_Dict[(a,b,f)]) <= 1
...
if OWFCR_Type==2:
    for j in Turbines_Index_Set:
        prob += lpSum(y[i,j] for i in All_Nodes_Index_Set if i!=j) == lpSum(d * z[j,d] for d in D)
...
if OWFCR_Type==3:
    for h in (Turbines_Index_Set | Steiners_Index_Set):
        prob += lpSum(y[i,h]+y[h,i]+q[i,h]+q[h,i] for i in All_Nodes_Index_Set if i!=h) == 2*lpSum(y[h,j] for j in All_Nodes_Index_Set if j!=h)
    #mdl.addConstrs(quicksum(y[i,h]+y[h,i]+q[i,h]+q[h,i] for i in All_Nodes_Index_Set if i!=h) == 2 * quicksum() for h in (Turbines_Index_Set | Steiners_Index_Set))
    #mdl.update()
    ...
    for Each_Crossing in Crossings_Set:
        ...
        i , j = Line2
        prob += y[i,j] + y[j,i] + y[h,k] + y[k,h] <= 1
        prob += y[i,j] + y[j,i] + y[h,k] + y[k,h] + q[i,j] + q[j,i] + q[h,k] + q[k,h] <= 1
        #mdl.addConstr(y[i,j] + y[j,i] + y[h,k] + y[k,h] <= 1)
...
if OWFCR_Type==4:
    for h in Turbines_Index_Set:
        prob += lpSum(x[i,h,t] for i in All_Nodes_Index_Set if i!=h for t in Cables_Index_Set if t!=Tao) <= mu_1
    ...
    for h in Turbines_Index_Set:
        prob += lpSum(x[i,h,Tao] for i in All_Nodes_Index_Set if i!=h) <= mu_2
...
start_time=time()
#status=prob.solve(p.PULP_CBC_CMD(maxSeconds=TimeLimit, msg=1, gapRel=0))
status=prob.solve(PULP_CBC_CMD(timeLimit=TimeLimit))
#status=prob.solve()
end_time=time()
...
for i in All_Nodes_Index_Set:
    for j in All_Nodes_Index_Set:
        if i!=j:
            ...
            cell.value = j
            cell = sheet_individual.cell(row = row_number_on_Individual_Sheet, column = 3)
            cell.value = value(y[i,j])
            cell = sheet_individual.cell(row = row_number_on_Individual_Sheet, column = 4)
            cell.value = value(f[i,j])
            
            for col_number,t in enumerate(Cables_Index_Set):
                cell = sheet_individual.cell(row = row_number_on_Individual_Sheet, column = 5 + col_number)
                cell.value = value(x[i,j,t])

#wb_individual.save(str(directory_to_save_PuLP_CBC_solution)+"Solution Details.xlsx")
